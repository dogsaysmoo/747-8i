...
#        var fms = 0;
		...
		if (abs(change) > 500) {
			...
			if (change > 3000) {
				...
				if (1.1 * getprop("engines/engine[0]/rpm") > 88.0) {
				    me.thrust_mode.setValue(5);
				    me.thrust_setting.setValue(1.1 * getprop("engines/engine[0]/rpm"));
				} else {
				    me.thrust_mode.setValue(2);
				}
			    }
...
#            me.AP_roll_arm.setValue(msg);
	    ...
	    if (idx==5) {
#		me.thrust_setting.setValue(91.8);
		msg = "CON";
	    }
	    if (idx==6) {
	    ...
	    if (idx==8) {
		me.thrust_setting.setValue(58.0);
		msg = "IDLE";
	    }
		...
		if (me.ias_setting.getValue() - ias_now > 15) {
            ...
            if (getprop("controls/engines/engine[0]/reverser") or getprop("controls/engines/engine[1]/reverser") or getprop("controls/engines/engine[2]/reverser") or getprop("controls/engines/engine[3]/reverser")) {
...
setlistener("autopilot/internal/disconnect-signal", func (sig) {
    if (sig.getBoolValue()) {
        if (afds.AP.getBoolValue()) {
            afds.AP.setBoolValue(0);
        } else {
	    afds.autothrottle_mode.setValue(0);
        }
    }
},0,0);
